...
Fill entries in `A` by zeros and return `A`.
...
Return `x` if `x > zero(x)` holds and `zero(x)` otherwise. This rule implies that `zero(x)`
is returned if `x` is a *NaN*.

# See also

[`soft_thresholder`](@ref), [`ImageProcessing.fast_max`](@ref).
...
"""
    ImageProcessing.fast_max(x, y)

Return `x` if `x > y` holds and `y` otherwise.

This function is intended for fast computations (e.g., vectorized loops). If one of `x` or
`y` is a *NaN*, `y` is returned.

# See also

[`ImageProcessing.fast_min`](@ref).

"""
fast_max(x, y) = fast_max(promote(x, y)...)
fast_max(x::T, y::T) where {T} = x > y ? x : y
@public fast_max

"""
    ImageProcessing.fast_min(x, y)

yields `x` if `x < y` holds and `y` otherwise.
...
[`ImageProcessing.fast_max`](@ref).

"""
fast_min(x, y) = fast_min(promote(x, y)...)
fast_min(x::T, y::T) where {T} = x < y ? x : y
@public fast_min

"""
    ImageProcessing.quick_all_ones(A) -> bool

Return whether it can be quickly inferred that all values of array `A` are equal to one. The
test is meant to be fast, the result is based on the type of `A` and, at most, on a single
value of `A`.

# See also

[`ImageProcessing.default_weights`](@ref).
...
Retrun a fast uniform array of ones of same axes as array `A`.

# See also

[`ImageProcessing.quick_all_ones`](@ref).
...
Return a tuple of intervals representing object `x`.
...
Return the *end-points* `start` and `stop` of `S` such that for all `x ∈ S`:
...
Contrary to `extrema(S)` which throws for empty sets or collections, `endpoints(S)` attempts
to return `start` and `stop` if possible. Hence, `start` and `stop` may be such that the
above inequalities cannot hold for any `x` because, e.g., `S` is empty or contains unbounded
values such as `NaN`s.
...
_endpoints(::Base.HasEltype, ::Base.HasLength, iter) =
    # Iterator length and element type known: returns the endpoints of an empty interval if
    # length is zero, otherwise calls `extrema`.
...
Return whether `x` has integer coordinates. Note that floating-point coordinates are
...
Compare `a` and `b` with comparison operator `op` and as if `a` and `b` be Cartesian
coordinates. `a` and `b` can be tuples, Cartesian indices, or points.

For example, `compare_coordinates(a, <, b)` returns whether `a` is less than `b` considering
their elements in reverse order (from the last to the first ones). For tuples of Cartesian
indices, this corresponds to the ordering of array elements in column-major order (as
...
Return whether `x`, `lo`, and `hi` are tuples of same length and such that
`lo[i] ≤ x[i] ≤ hi[i]` holds for all indices `i`.
...
Return whether `x`, `lo`, and `hi` are tuples of at least one element and such that
`lo[1] ≤ x[1] ≤ hi[1]`.
...
Return whether `lo ≤ x ≤ hi` holds.

"""
is_between(x, lo, hi) = is_between(promote(x, lo, hi)...)
is_between(x::T, lo::T, hi::T) where {T} = lo ≤ x ≤ hi

"""
    ImageProcessing.front(t::Tuple)

Return the front part of tuple `t`, that is all elements of `t` but the last one keeping
their order.

# Also see

`Base.front` and [`ImageProcessing.tail`](@ref).
...
Return the tail part of tuple `t`, that is all elements of `t` but the first one keeping
their order.

# Also see

`Base.tail` and [`ImageProcessing.front`](@ref).

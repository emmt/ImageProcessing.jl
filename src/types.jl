...
Alias for n-tuples of at least one element of type `T`.
...
Alias to the union of types of objects `x` that can be directly converted into an interval
by calling the `Interval` constructor as `Interval(x)`. In other words, if `x isa
...
Abstract super-type of `N`-dimensional *points* having coordinates of type `T`. A point `p`
is also iterable and indexable with `i`-th coordinate given by `p[i]`. For a point `p`, the
number of coordinates given by `length(p)` is a *trait*: it is inferable from the type.

For concrete point types, say `PointType`, apart from constructors given the coordinates, at
least the method `Base.values(p::PointType)` must be specialized to yield an indexable (e.g.
vector or tuple) list of the coordinates of `p`. In the API for abstract points, points are
assumed to have 1-based indices. If this is not the case, the base methods `firstindex`,
`lastindex`, and `keys` must also be specialized. For example:
...
Alias to the union of types of objects `x` that can be directly converted into a
`N`-dimensional point by calling the `Point` constructor as `Point(x)` or `Point{N}(x)`. In
other words, if `x isa PointLike{N}` holds then `Point(x)` yields a `N`-dimensional point.
...
#      (2) `NTuple{N}` won't work because there is no guaranties that conversion to a point
#          is possible.
...
# Union of types to specify a position in an `N`-dimensional array with possibly fractional
# coordinates.
...
struct Point{N,T} <: AbstractPoint{N,T}
    ...
    # coordinates if needed. Since argument is an `N`-tuple, parameter `N` is guaranteed to
    # be an `Int`, no needs to check.
...
Alias to the union of types of objects `x` that can be directly converted into a
`N`-dimensional bounding-box by calling the `BoundingBox` constructor as `BoundingBox(x)` or
`BoundingBox{N}(x)`. In other words, if `x isa BoundingBoxLike{N}` holds then
